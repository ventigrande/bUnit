...
1. Creates a new instance of the disposable bUnit <xref:Bunit.>, and assigns it to `ctx` variable using the `using var` syntax to avoid unnecessary source code indention.
2. Renders the `<HelloWorld>` component using <xref:Bunit.TestContext>, which is done through the <xref:Bunit.Extensions.TestRendererExtensionsTestContext.RenderComponent``1(Bunit.Rendering.ITestRenderer,System.Action{Bunit.ComponentParameterCollectionBuilder{``0}})> method. We cover passing parameters to components on the <xref:passing-parameters-to-components> page.
...
Then methods like <xref:Bunit.Extensions.TestRendererExtensionsTestContext.RenderComponent``1(Bunit.Rendering.ITestRenderer,System.Action{Bunit.ComponentParameterCollectionBuilder{``0}})> can be called directly from each test, as seen in the listing above.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MzQ1MDI5MDhdfQ==
-->